...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.input_embeddings = nn.Embedding(vocab_size, model_dim)
        self.position_embeddings = nn.Embedding(context_length, model_dim)
        self.attention_blocks = nn.Sequential()
        for _ in range(num_blocks):
            self.attention_blocks.append(self.TransformerBlock(model_dim, num_heads))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embed = self.input_embeddings(context)
        pos = torch.arange(context_length, device= context.device)
        embed += self.position_embeddings(pos)
        x = self.attention_blocks(embed)
        x = self.layer_norm(x)
        x = self.proj(x)
