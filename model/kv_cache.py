...
class CachedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> Tuple[torch.Tensor, KVCache]:
        ...
        scores = torch.einsum("bid, bjd -> bij", Q, k_cached) / math.sqrt(Q.shape[-1])
        scores = nn.functional.softmax(scores, dim=-1) @ v_cached
        return (torch.round(scores,decimals=4), kv_cache)
